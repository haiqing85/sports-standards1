...
HBBA_KEYWORDS = ["体育", "运动场地", "健身器材", "游泳", "体育场馆照明", "人造草", "塑胶跑道"]
DBBA_KEYWORDS = [
    ...
    try:
        ...
        for row in rows:
            ...
            items.append({
                ...
                "replaces":      None,
                "replacedBy":    None,
                "isMandatory":   bool(re.match(r"^(GB|TY|DB\d+)\d", re.sub(r"\s+","",code).upper()) and "/T" not in code),
                "summary":       "",
                "tags":          [],
                "category":      "综合",
                "scope":         "",
...
if __name__ == "__main__":
    ...
    for x in r[:2]: print(json.dumps(x, ensure_ascii=False))
