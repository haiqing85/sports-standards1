#!/usr/bin/env python3
"""
体育建设标准自动抓取 & 更新脚本 v9.1（bug修复版）
======================================
v9.1 修复：
  1. 修复核心数据加载bug，正确读取standards.json的标准数组
  2. 全流程增加类型容错，彻底解决str对象无get属性报错
  3. 修复数据保存格式，正确写入带元数据的完整JSON结构
  4. 强化体育相关过滤，彻底排除非体育/球类无关内容
  5. 支持多发布单位拼接，完整展示联合发布机构
  6. 精准化摘要生成规则，AI摘要提示更精准
  7. 提升搜索完整性，扩大分页抓取范围，匹配官方平台结果数量
"""
...
# ============================================================
def load_existing():
    """加载已有标准库，正确读取standards数组，增加格式容错"""
    if DATA_FILE.exists():
        try:
            # 读取整个JSON文件
            full_data = json.loads(DATA_FILE.read_text(encoding='utf-8'))
            # 正确提取standards数组，兼容新旧格式
            if isinstance(full_data, dict):
                standards_list = full_data.get('standards', [])
                # 确保返回的是列表
            ...
            # 如果是旧格式的纯列表，直接返回
                ...
                return full_data
            # 格式错误，返回空列表
            log("⚠️  现有数据文件格式异常，将重建")
            return []
        except Exception as e:
            log(f"⚠️  现有数据文件读取失败: {e}，将重建")
    return []

def clean_non_sports(standards):
    """清理库中非体育标准，增加类型容错，彻底解决str报错"""
    if not isinstance(standards, list):
        log("⚠️  传入的标准数据不是列表，返回空列表")
        return []
    before = len(standards)
    # 先过滤非字典元素，再过滤非体育标准
    valid_standards = [s for s in standards if isinstance(s, dict)]
    filtered = [s for s in valid_standards if is_sports(s.get('title',''))]
    after = len(filtered)
    invalid_count = before - len(valid_standards)
    if invalid_count > 0:
        log(f"🧹 过滤无效非字典数据：{invalid_count} 条")
    log(f"🧹 清理非体育标准：{len(valid_standards)} → {after} 条")
    return filtered

def save_standards(standards, dry_run=False):
    """保存标准库，正确写入完整JSON结构，兼容原文件格式"""
    today = datetime.now().strftime('%Y-%m-%d')
    version = today.replace('-','.')
    # 构建完整的JSON结构，和原文件格式完全匹配
    full_data = {
        "version": version,
        "updated": today,
        "total": len(standards),
...
def main():
    ...
    log("🌐 开始抓取各来源标准…")
    all_new = []
    seen_codes = set([norm_code(s.get('code','')) for s in existing if isinstance(s, dict)])
    ...
    # 统计信息
    miss_issued   = sum(1 for s in final if isinstance(s, dict) and not s.get('issuedBy'))
    miss_summary  = sum(1 for s in final if isinstance(s, dict) and not s.get('summary','').strip())
    miss_replaces = sum(1 for s in final if isinstance(s, dict) and s.get('status') == '废止' and not s.get('replacedBy'))
    active = sum(1 for s in final if isinstance(s, dict) and s.get('status')=='现行')
    abol   = sum(1 for s in final if isinstance(s, dict) and s.get('status')=='废止')
    coming = sum(1 for s in final if isinstance(s, dict) and s.get('status')=='即将实施')

    log("="*60)
    log("✅ 全部完成！")
    log(f"📊 最终统计：总 {len(final)} 条 | 现行 {active} | 废止 {abol} | 即将实施 {coming}")
    log(f"\n📋 字段完整性：")
