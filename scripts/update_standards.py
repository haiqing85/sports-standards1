#!/usr/bin/env python3
"""
体育标准数据库 — 自动抓取更新 v12（球类全覆盖+20页限制版）
更新：新增皮克球、台球 + 全球类关键词 + 每页限制20页
"""
...
def guess_category(text):
    cm = {"体育馆":"场地设计","人造草":"人造草坪","木质地板":"木地板","塑胶":"合成材料面层","照明":"灯光照明","围网":"围网","健身":"健身路径"}
    for k,v in cm.items():
        if k in text: return v
    return "综合"

def guess_tags(text):
    return [t for t in ["体育","运动","体育馆","人造草","木质地板","塑胶","照明","木地板","围网","健身"] if t in text][:6]
...
def build_entry(item):
    ...
    return {
        ...
        'summary': '', 'isMandatory': is_mandatory(code), 'scope': '', 'localFile': None
    }
    ...
    for domain in domains:
        try:
            ...
            for row in rows:
                ...
                issued_by = row.get('ISSUE_DEPT') or row.get('C_ISSUE_DEPT') or ''
...
def merge(existing, new_items):
    ...
    for item in new_items:
        ...
        if nc in idx:
            o = existing[idx[nc]]
            for f in ['status','issueDate','implementDate','abolishDate','replaces','replacedBy','issuedBy']:
                v = item.get(f)
                if v: o[f] = v
            upd +=1
        else:
